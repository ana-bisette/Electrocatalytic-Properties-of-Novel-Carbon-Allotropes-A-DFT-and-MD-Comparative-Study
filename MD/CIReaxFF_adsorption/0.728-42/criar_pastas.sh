...
for i in "H" "O" "OH" "OOH"; do
	for j in $(seq 1 3); do
		...
		mv ${i}_${j}.lmp ${i}_${j}
		cp -r /home/ana.bisette/jobs/scripts_n_tests/fields/new_reaxff/. /home/ana.bisette/jobs/adsorption/0.728-42/${i}_specie/${i}_${j}/.
		cd /home/ana.bisette/jobs/adsorption/0.728-42/${i}_specie/${i}_${j}
                vim -E -s input <<-EOF
                :%s/x.lmp/${i}_${j}.lmp/g
                :update
                :quit
...
for x in $(seq 1 3 ); do
	cd /home/ana.bisette/jobs/adsorption/0.728-42/H_specie/H_${x}
	vim -E -s input <<-EOF
	:%s/C H O/C H/g
	:update
	:quit
...
for x in $(seq 1 3 ); do
        cd /home/ana.bisette/jobs/adsorption/0.728-42/O_specie/O_${x}
        vim -E -s input <<-EOF
        :%s/C H O/C O/g
        :update
        :quit
