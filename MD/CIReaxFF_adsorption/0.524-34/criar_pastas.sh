...
for i in "H" "O" "OH" "OOH"; do
	for j in $(seq 1 2); do
		...
		mv ${i}_${j}.lmp ${i}_${j}
		cp -r /home/ana.bisette/jobs/scripts_n_tests/fields/new_reaxff/. /home/ana.bisette/jobs/adsorption/0.524-34/${i}_specie/${i}_${j}/.
...
for x in $(seq 1 2); do
	cd /home/ana.bisette/jobs/adsorption/0.524-34/H_specie/H_${x}
	vim -E -s input <<-EOF
	:%s/C H O/C H/g
	:update
	:quit
...
for x in $(seq 1 2); do
        cd /home/ana.bisette/jobs/adsorption/0.524-34/O_specie/O_${x}
        vim -E -s input <<-EOF
        :%s/C H O/C O/g
        :update
        :quit
